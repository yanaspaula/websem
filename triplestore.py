...
def loadDatatoServer():
    """ Télécharger les données à partir des différentes sources
        Créer les triples associés --> Graphs RDF
        Enregistrer les graphes sous format ttl et sous format JSON LD
        Stocker les triples sur le serveur Fuseki
        Cette fonction s'exécute toutes les deux minutes afin de mettre à jour les données """

    # Mettre à jour les données en faisant appel toutes les deux minutes à la fonction loadDatatoServer
    threading.Timer(120.0, loadDatatoServer).start() 
    ...
    graphE.serialize(format="json-ld", indent=4, destination="df_bornes.json")
    ...
    graphE.serialize(format="json-ld", indent=4, destination="df_parking.json")
